 ...
 * @param {string} apiKey the API key to use to authenticate against Docker Cloud.
 ...
 */
function findServicesByLabel(label, apiKey, apiLimit, callback) {
  ...
  fetch(`https://cloud.docker.com/api/app/v1/service/?limit=${apiLimit}`, {
    ...
    .then(services => {
      async.reduce(services.objects, [], (res, item, cbk) => {
          _fetchService(item.uuid, apiKey, (err, service) => {
            if (err) return cbk(err);
            else if (service.labels.hasOwnProperty(label)) {
...
function _fetchService(uuid, apiKey, callback) {
 ...
 */
function startService(uuid, apiKey, callback) {
